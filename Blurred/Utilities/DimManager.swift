...
extension DimManager {
  ...
  private func windows(color: NSColor, withDelay: Bool, didCreateWindows: @escaping ([NSWindow]) -> Void) {
  ...
  private func windowForScreen(screen: NSScreen, windowInfos: [WindowInfo], color: NSColor) -> NSWindow {
    ...
    let overlayWindow = NSWindow.init(
    ...
    case .parallel:
      ...
      let newScreen = NSRect(
        x: screen.frame.minX, y: NSScreen.screens[0].frame.maxY - screen.frame.maxY, width: screen.frame.width,
        height: screen.frame.height)
      let windowInfo = windowInfos.first(where: {
        return newScreen.minX <= $0.bounds!.midX && newScreen.maxX >= $0.bounds!.midX
          && newScreen.minY <= $0.bounds!.midY && newScreen.maxY >= $0.bounds!.midY
      })
...
extension DimManager {
  ...
  private func observerActiveWindowChanged() {
    ...
    nc.addObserver(
      self, selector: #selector(workspaceDidReceiptAppllicatinActiveNotification),
